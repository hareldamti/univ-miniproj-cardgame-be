...
export function initializeGame(usernames: string[], layout: number): GameState {
    const players = initializePlayers(usernames);
    return {
        Table: initializeTable(layout),
        players: players,
...
//Initialize Table
export function initializeTable(layout: number): Table {
    return {
        Board: initializeBoard(layout),
...
// Initialize Board
function initializeBoard(layout: number): Hexagonal[][] {
    const partialBoard = partialBoards[layout];
